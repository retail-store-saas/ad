...
public final class AdService {
    ...
    private class AdServiceHttpHandler implements HttpHandler {
        @Override
        public void handle(HttpExchange httpExchange) throws IOException {
            ...
            if (System.getenv("VULN_LOG4SHELL_ENABLED") != null) {
            ...
            try {
                ...
                if (httpExchange.getRequestURI().getQuery() != null) {
                    Map<String, String> reqParameter = splitQuery(httpExchange.getRequestURI().getQuery()); 
                    String adCategoreisParam = reqParameter.getOrDefault("categories", "");

                    String[] adCategories  = adCategoreisParam.split(",");
                    if (adCategories.length > 0) {
                        for (int i = 0; i < adCategories.length; i++) {
                            Collection<Ad> ads = service.getAdsByCategory(adCategories[i]);
                            allAds.addAll(ads);
                        }
                    } else {
                ...
                jsonResponse = new Gson().toJson(allAds);
            } catch (Exception e) {
                jsonResponse = "{\"error\": \"failed to obtain ad inforamation\"}";
            }
    ...
    private void start() throws IOException {
        ...
        httpServer.createContext("/ads", new  AdServiceHttpHandler());
